        ...
        <Modal title="Assign Membership Plan" onClose={() => { if (!busy) setModal(null); }} footer={<><button className="btn btn-secondary" onClick={() => setModal(null)} disabled={busy}>Cancel</button><button className="btn btn-primary" onClick={assign} disabled={busy}>{busy ? <><RefreshCw size={14} style={{ animation: "spin 1s linear infinite" }} /> Saving...</> : <><Check size={14} /> Assign & Record Payment</>}</button></>}>
          ...
          <div className="form-grid">
            ...
            {/* ADMIN-ONLY: backdate the membership for record migration.
                Native <input type="date"> with forced showPicker() — the
                browser hands us YYYY-MM-DD directly so we no longer need
                the DD/MM/YYYY parsing dance. Empty value means "use today"
                (the backend default kicks in). max=today blocks the picker
                from offering future dates entirely. */}
            {isAdmin && (() => {
              const parsed = form.startDate; // YYYY-MM-DD or ""
              const hasInput = !!parsed;
              const isValid = hasInput && parsed <= today();
              return (
                <div className="form-group full">
